...
<#macro showMsg returnPath="">
	...
	<#else>
		<#if successMsg??>
			...
			<#else>
				<div class="am-alert am-alert-success" data-am-alert=""><button type="button" class="am-close">×</button><p>${successMsg}<br /> 您可以点击 <a href="${springMacroRequestContext.getContextUrl(returnPath)}" class="color_Blue">这里</a> 返回列表</p></div>
			</#if>
		</#if>
	</#if>
</#macro>
